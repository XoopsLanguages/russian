<?php
// 
// _LANGCODE: ru
// _CHARSET : UTF-8
// Translator: XOOPS Translation Team
//%%%%%%        File Name user.php         %%%%%
define('_US_NOTREGISTERED', 'Вы не зарегистрированы?  Нажмите <a href="register.php">здесь</a>.');
define('_US_LOSTPASSWORD', 'Забыли свой пароль?');
define('_US_NOPROBLEM', 'Нет проблем. Просто введите e-mail Вашей учетной записи.');
define('_US_YOUREMAIL', 'Ваш Email: ');
define('_US_SENDPASSWORD', 'Выслать пароль');
define('_US_LOGGEDOUT', 'Ваш сеанс закрыт');
define('_US_THANKYOUFORVISIT', 'Благодарим Вас за посещение нашего сайта!');
define('_US_INCORRECTLOGIN', 'Неверный логин!');
define('_US_LOGGINGU', 'Спасибо за визит на наш сайт, %s.');
// 2001-11-17 ADD
define('_US_NOACTTPADM', 'Выбранный пользователь был дезактивирован или еще не был активирован.<br>Пожалуйста свяжитесь с администратором для уточнения.');
define('_US_ACTKEYNOT', 'Ключ активации не верный!');
define('_US_ACONTACT', 'Выбранная учетная запись уже активирована!');
define('_US_ACTLOGIN', 'Ваша учетная запись была активирована. Пожалуйста, войдите с зарегистрированным паролем.');
define('_US_NOPERMISS', 'К сожалению, у Вас нет разрешения на выполнение этого действия!');
define('_US_SURETODEL', 'Вы уверены, что хотите удалить свой аккаунт?');
define('_US_REMOVEINFO', 'Это удалит все Ваши данные из нашей базы данных.');
define('_US_BEENDELED', 'Ваш аккаунт был удален.');
define('_US_ACTFAILD', 'Активация не удалась!');
//%%%%%%        File Name register.php         %%%%%
define('_US_USERREG', 'Регистрация пользователя');
define('_US_NICKNAME', 'Имя пользователя');
define('_US_EMAIL', 'Email');
define('_US_ALLOWVIEWEMAIL', 'Разрешить другим пользователям просматривать мой адрес электронной почты');
define('_US_WEBSITE', 'ВебСайт');
define('_US_TIMEZONE', 'Часовой пояс');
define('_US_AVATAR', 'Аватар');
define('_US_VERIFYPASS', 'Подтвердите пароль');
define('_US_SUBMIT', 'Отправить');
define('_US_USERNAME', 'Имя пользователя');
define('_US_FINISH', 'Конец');
define('_US_REGISTERNG', 'Не удалось зарегистрировать нового пользователя.');
define('_US_MAILOK', 'Периодически получать уведомления на E-mail<br>от администраторов и модераторов?');
define('_US_DISCLAIMER', 'Отказ от ответственности');
define('_US_IAGREE', 'Я согласен с выше написанным');
define('_US_UNEEDAGREE', 'К сожалению, Вы должны согласиться на наш отказ от ответственности, чтобы зарегистрироваться.');
define('_US_NOREGISTER', 'К сожалению, мы в настоящее время закрыты для регистрации новых пользователей');
// %s is username. This is a subject for email
define('_US_USERKEYFOR', 'Ключ активации для пользователя %s');
define('_US_YOURREGISTERED', 'Теперь Вы зарегистрированы. На введенный Вами при регистрации E-mail адрес выслано сообщение содержащее ключ активации пользователя. Пожалуйста следуйте инструкциям, указанным в E-mail сообщении для активации учетной записи.');
define('_US_YOURREGMAILNG', 'Теперь Вы зарегистрированы. Однако, мы не смогли выслать ключ активации на Ваш E-mail адрес по причине внутренней ошибки на нашем сервере. Мы приносим свои извинения за причиненные неудобства, пожалуйста свяжитесь с администратором сайта по E-mail для решения возникшей проблемы.');
define('_US_YOURREGISTERED2', 'Теперь Вы зарегистрированы.  Пожалуйста подождите пока Ваша учетная запись не будет активирована администратором. Вы получите E-mail сообщение когда это произойдет. Это может занять некоторое время, поэтому будьте терпеливы.');
// %s is your site name
define('_US_NEWUSERREGAT', 'Регистрация нового пользователя на %s');
// %s is a username
define('_US_HASJUSTREG', '%s только что зарегистрирован!');
define('_US_INVALIDMAIL', 'ОШИБКА: Неверный E-mail');
define('_US_EMAILNOSPACES', 'ОШИБКА: E-mail адреса не содержат пробелов.');
define('_US_INVALIDNICKNAME', 'ОШИБКА: Неверное имя пользователя');
define('_US_NICKNAMETOOLONG', 'Имя пользователя слишком длинное. Оно должно быть меньше, чем %s символов.');
define('_US_NICKNAMETOOSHORT', 'Имя пользователя слишком короткое. Оно должно быть больше, чем %s символов.');
define('_US_NAMERESERVED', 'ОШИБКА: Имя зарезервировано.');
define('_US_NICKNAMENOSPACES', 'Имя пользователя не должно содержать пробелов.');
define('_US_NICKNAMETAKEN', 'ОШИБКА: Имя пользователя занято.');
define('_US_EMAILTAKEN', 'ОШИБКА: E-mail адрес уже зарегистрирован.');
define('_US_ENTERPWD', 'ОШИБКА: Вы должны указать пароль.');
define('_US_SORRYNOTFOUND', 'К сожалению, соответствующей информации о пользователе не было найдено.');
// %s is your site name
define('_US_NEWPWDREQ', 'Новый запрос пароля для %s');
define('_US_YOURACCOUNT', 'Ваш аккаунт в %s');
define('_US_MAILPWDNG', 'mail_password: не удалось обновить запись пользователя. Обратитесь к администратору');
// %s is a username
define('_US_PWDMAILED', 'Пароль для %s отправлен.');
define('_US_CONFMAIL', 'Подтверждение по E-mail для %s отправлено.');
define('_US_ACTVMAILNG', 'Ошибка отправки уведомлений по E-mail к %s');
define('_US_ACTVMAILOK', 'Уведомление по E-mail к %s отправлено.');
//%%%%%%        File Name userinfo.php         %%%%%
define('_US_SELECTNG', 'Пользователь не выбран! Пожалуйста вернитесь и попробуйте снова.');
define('_US_PM', 'PM');
define('_US_ICQ', 'ICQ');
define('_US_AIM', 'AIM');
define('_US_YIM', 'YIM');
define('_US_MSNM', 'MSNM');
define('_US_LOCATION', 'Место нахождения');
define('_US_OCCUPATION', 'Род занятий');
define('_US_INTEREST', 'Интересы');
define('_US_SIGNATURE', 'Подпись');
define('_US_EXTRAINFO', 'Дополнительная информация');
define('_US_EDITPROFILE', 'Редактировать профиль');
define('_US_LOGOUT', 'Выход');
define('_US_INBOX', 'Входящие');
define('_US_MEMBERSINCE', 'Участник с');
define('_US_RANK', 'Ранг');
define('_US_POSTS', 'Комментарии/Сообщения');
define('_US_LASTLOGIN', 'Последний вход');
define('_US_ALLABOUT', 'Все о %s');
define('_US_STATISTICS', 'Статистика');
define('_US_MYINFO', 'Моя информация');
define('_US_BASICINFO', 'Основная информация');
define('_US_MOREABOUT', 'Больше обо мне');
define('_US_SHOWALL', 'Показать все');
//%%%%%%        File Name edituser.php         %%%%%
define('_US_PROFILE', 'Профиль');
define('_US_REALNAME', 'Реальное имя');
define('_US_SHOWSIG', 'Всегда присоединять мою подпись');
define('_US_CDISPLAYMODE', 'Режим отображения комментариев');
define('_US_CSORTORDER', 'Сортировать комментарии');
define('_US_PASSWORD', 'Пароль');
define('_US_TYPEPASSTWICE', '(введите новый пароль дважды, чтобы изменить его)');
define('_US_SAVECHANGES', 'Сохранить изменения');
define('_US_NOEDITRIGHT', "К сожалению, Вы не имеете право редактировать данные этого пользователя.");
define('_US_PASSNOTSAME', 'Оба пароля различны. Они должны быть одинаковыми.');
define('_US_PWDTOOSHORT', 'К сожалению, Ваш пароль должен быть по крайней мере <strong>%s</strong> символов.');
define('_US_PROFUPDATED', 'Ваш профиль обновлен!');
// removed in 2.5.9 define('_US_USECOOKIE', 'Хранить мое имя пользователя в куки на 1 год');
define('_US_NO', 'Нет');
define('_US_DELACCOUNT', 'Удалить аккаунт');
define('_US_MYAVATAR', 'Мой аватар');
define('_US_UPLOADMYAVATAR', 'Загрузить аватар');
define('_US_MAXPIXEL', 'Max Pixels');
define('_US_MAXIMGSZ', 'Max размер изображения (Bytes)');
define('_US_SELFILE', 'Выберите файл');
define('_US_OLDDELETED', 'Ваш старый аватар будет удален!');
define('_US_CHOOSEAVT', 'Выберите аватар из списка доступных');
define('_US_PRESSLOGIN', 'Нажмите на кнопку ниже, чтобы войти в систему');
define('_US_ADMINNO', 'Пользователь в группе вебмастеров не могут быть удалены');
define('_US_GROUPS', 'Группы пользователей');
define('_US_REMEMBERME', 'Запомнить меня');
// Welcoming emai/PM subject
define('_US_WELCOME_SUBJECT', 'Добро пожаловать в %s');

//XOOPS 2.5.4
define('_US_SKYPE', 'Skype');
define('_US_FACEBOOK', 'Facebook');
//XOOPS 2.5.9
define('_US_USERALREADYACTIVE', 'Пользователь уже активен');
