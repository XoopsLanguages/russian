<?php
/**
 * @copyright       (c) 2000-2016 XOOPS Project (www.xoops.org)
 * @license         GNU GPL 2 or later (http://www.gnu.org/licenses/gpl-2.0.html)
 * _LANGCODE    ru
 * _CHARSET     UTF-8
 */
//Nav
define('_AM_SYSTEM_TEMPLATES_NAV_MANAGER', 'Управление темами');
define('_AM_SYSTEM_TEMPLATES_NAV_MAIN', 'Управление темами');
//Tips
define('_AM_SYSTEM_TEMPLATES_NAV_TIPS', '
<ul>
<li>Редактирование темы и шаблонов модулей, таблица стилей в Интернете.</li>
<li>Создать все модули переопределения шаблонов.</li>
<li>Если вы устаналиваете поколение, это сотрет все предыдущие модификации.</li>
</ul>
');
define('_AM_SYSTEM_TEMPLATES_YOUR_THEMES', 'Ваши темы :');
define('_AM_SYSTEM_TEMPLATES_GENERATE', 'Сгенерировать шаблон');
define('_AM_SYSTEM_TEMPLATES_SELECT_THEME', 'Выберите тему');
define('_AM_SYSTEM_TEMPLATES_FORCE_GENERATED', 'Принудительная генерация файла');
define('_AM_SYSTEM_TEMPLATES_NAV_FILE_GENERATED', 'Файлы генерируются');
define('_AM_SYSTEM_TEMPLATES_NOT_CREATED', 'Файлы не созданы');
define('_AM_SYSTEM_TEMPLATES_SAVE', 'Сохранить');
define('_AM_SYSTEM_TEMPLATES_RESTORE', 'Восстановить');
define('_AM_SYSTEM_TEMPLATES_CANCEL', 'Отмена');
define('_AM_SYSTEM_TEMPLATES_ERROR', 'Ошибка');
define('_AM_SYSTEM_TEMPLATES_EDIT', 'Редактировать :');
define('_AM_SYSTEM_TEMPLATES_SELECT_MODULES', 'Выбор модуля');
define('_AM_SYSTEM_TEMPLATES_MODULES', 'Модуль : ');
define('_AM_SYSTEM_TEMPLATES_FILES', 'Файл');
define('_AM_SYSTEM_TEMPLATES_EMPTY_FILE', 'Пустой файл');
define('_AM_SYSTEM_TEMPLATES_TYPES', 'Тип');
define('_AM_SYSTEM_TEMPLATES_STATUS', 'Статус');
define('_AM_SYSTEM_TEMPLATES_TEMPLATES', 'Шаблон');
define('_AM_SYSTEM_TEMPLATES_BLOCKS', 'Блок');
define('_AM_SYSTEM_TEMPLATES_SELECT_TEMPLATES', 'Выбрать шаблон');
define('_AM_SYSTEM_TEMPLATES_ALL_MODULES', 'Все модули');
define('_AM_SYSTEM_TEMPLATES_RESTORE_OK', 'Успешно восстановлен');
define('_AM_SYSTEM_TEMPLATES_RESTORE_NOTOK', 'Не удалось восстановить');
define('_AM_SYSTEM_TEMPLATES_SET', 'Выберите шаблон');
